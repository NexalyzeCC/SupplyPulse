...
function EmptySignals({ filtered }: { filtered: boolean }) {
  return (
    <EmptyState
      size="sm"
      icon={Radio}
      title={filtered ? "No signals match this filter" : "No signals detected"}
      description={
        filtered
          ? "Try selecting a different signal type above."
          : "The agent found no relevant risk signals in this scan. Run a new scan to check for recent events."
      }
    />
  );
}
